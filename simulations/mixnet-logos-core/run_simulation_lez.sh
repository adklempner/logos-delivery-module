...
# Generate credentials for chat2mix only (logoscore nodes use gifter protocol)
...
for i in $(seq 0 $((NUM_NODES - 1))); do
    ...
    if [ "$i" -eq 0 ]; then
        ...
        GIFTER_FIELDS="\"mixGifterNode\": \"$BOOTSTRAP_PEER\","
    fi
...
EOF
    ...
    if [ "$i" -eq 0 ]; then
        # Node 0: gifter service — self-registers via RLN module, then serves other nodes
...
for i in $(seq 0 $((NUM_NODES - 1))); do
    LOG_FILE="$STATE_DIR/node${i}.log"
    if [ "$i" -eq 0 ]; then EC=6; else EC=4; fi
...
# Logoscore nodes register via gifter protocol (no register_commitments needed).
# Chat2mix credentials are registered below via setup_credentials keystores.
