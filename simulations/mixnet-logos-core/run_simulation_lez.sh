...
# Generate credentials BEFORE starting nodes so we can pass idSecretHash to each node
...
EOF
    ...
    # Credentials are loaded from keystores (generated by setup_credentials).
...
# Register identity commitments on-chain (credentials were generated before node startup)
