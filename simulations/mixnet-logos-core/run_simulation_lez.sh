...
# Generate credentials for all participants. Logoscore nodes use keystores for identity;
# chat2mix uses rln_tree.db for off-chain proof generation (must match on-chain tree).
...
EOF
    ...
    # All nodes load identity credentials from keystores.
    # Node 0 mounts the gifter service; nodes 1-3 are gifter clients that
    # register their keystore credentials on-chain via the gifter during start().
...
# Run chat2mix from STATE_DIR so it can access keystores + rln_tree.db (off-chain proof generation)
(printf 'receiver\n'; sleep 999) | \
